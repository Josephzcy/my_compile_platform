...
#include "common/config/xml_configer.hpp"
...
int  XMLConfiger::GetAllSubElementNum(const std::string& strKey)
{
    ...
    int  dwKeyLen = static_cast<int>(strKey.size());
                                      ...
                                      std::set<std::string>& sSubElement)
{
    ...
    for (it = m_mConfig.begin(); it != m_mConfig.end(); it++)
    {
        if (it->first.substr(0, dwKeyLen) == strKey)
        {
            dwPos = static_cast<int>(it->first.find(m_strSeparator, dwKeyLen+2));
            if (-1 != dwPos)
            {
...
{
#ifdef _MSC_VER
    value = static_cast<float>(strtod(str, endpos));
#else
...
/* 得到多个整型的配置项的值*/
template <typename Type>
bool GetMultiConfigValue(
    const std::multimap<std::string, std::string> config,
...
/* 得到多个字符串类型的配置项的值*/
bool XMLConfiger::GetMultiValue(const std::string& strKey,
                                std::vector<std::string>& vStrValue) const
{
    std::multimap<std::string, std::string>::const_iterator it;
    bool bRet = false;
...
bool XMLConfiger::GetMultiValue(
    const std::string& strKey,
    ...
    while (current != NULL)
    {
        ...
        else
        {
            ...
            if (temp != NULL)
            {
                ...
                //
                if (temp->NameLength == current->NameLength &&
                    memcmp(temp->Name, current->Name, current->NameLength) == 0)
                {
                    //
    ...
    while (field != NULL)
    {
        ...
        if (field->datalength != 0 && memcmp(field->data, "?", 1) != 0)
        {
            ...
            else
            {
                ...
                // If we look for the '>' we can find the end of this element
                temp2 = ParseString(field->data, 0, field->datalength, ">", 1);
                if (temp2->FirstResult->datalength > 0 &&
                    temp2->FirstResult->data[temp2->FirstResult->datalength - 1] == '/')
                {
            ...
            if (TagNameLength != 0)
            {
                ...
                if (EmptyTag != 0)
                {
                    ...
                    // just so the tree is consistent. No point in introducing unnecessary
                    // complexity
                    x = (struct XMLNode*)malloc(sizeof(struct XMLNode));
                    memset(x, 0, sizeof(struct XMLNode));
                    x->Name = TagName;
...
int XMLConfiger::GetAllTags(
    struct XMLNode *nodeList,
    std::multimap<std::string, std::string>& mXmlTags)
{
    int RetVal = 0;
    struct XMLNode *current = nodeList;
    struct XMLNode *temp;
