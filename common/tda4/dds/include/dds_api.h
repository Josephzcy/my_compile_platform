...
class Context {
  ...
  */
  ...
  @param enable_thread[in]: when you want to use notify mode, you should
      ...
      function.
  */
  explicit Context(const std::string& config_content, ContentHint content_hint, bool enable_thread = false);
  ...
  */
  ...
  /*
  @brief: Disbale inner event loop thread, after call this function,
    you will not recv any message from Writer.
  */
  void DisableEventLoop();

  /*
  @brief: Enable inner event loop thread.
  */
  void EnableEventLoop();

  // Context(const Context& c) = delete; // 禁止拷贝构造函数
  // Context& operator=(const Context& c) = delete; // 禁止拷贝赋值函数

  // Context(Context&& c) = delete; // 禁止转移构造函数
  // Context& operator=(Context&& c) = delete; // 禁止转移赋值函数

 public:
  DDSContext* m_ctx;
  bool m_enable_thread;
  bool m_enable_event_loop;
  std::thread m_thread;
};
...
class Writer {
  ...
  */
  ...
  // Writer(const Writer& w) = delete; // 禁止拷贝构造函数
  // Writer& operator=(const Writer& w) = delete; // 禁止拷贝赋值函数

  // Writer(Writer&& w) = delete; // 禁止转移构造函数
  // Writer& operator=(Writer&& w) = delete; // 禁止转移赋值函数
  ...
  */
  void* GetLatestPtr(size_t* id = nullptr, size_t* index = nullptr, size_t* size = nullptr);
...
class Reader {
  ...
  */
  explicit Reader(Context* ctx, const std::string& topic, DDS_OnDataCallBack callback = nullptr,
  ...
  */
  ...
  // Reader(const Reader& r) = delete; // 禁止拷贝构造函数
  // Reader& operator=(const Reader& r) = delete; // 禁止拷贝赋值函数

  // Reader(Reader&& r) = delete; // 禁止转移构造函数
  // Reader& operator=(Reader&& r) = delete; // 禁止转移赋值函数
  ...
  */
  ...
  @return On success: 0
          On error: error code is returned.
  */
  int ReadLatest(void* buf, size_t* len, size_t* id = nullptr, size_t* index = nullptr);
  ...
  */
  ...
  @return On success: 0
          On error: error code is returned.
  */
  int ReadByIDTimeout(size_t id, void* buf, size_t* len, size_t* index = nullptr, uint64_t timeout_millis = 1000,
  ...
  @return On success: 0
          On error: error code is returned.
  */
  int ReadPolling(size_t* check_id, void* buf, size_t* len, size_t* index = nullptr, uint64_t timeout_millis = 1000,
                  uint64_t sleep_millis = 100);
  ...
  @return On success: 0
          On error: error code is returned.
  */
  int ReadNotify(void* buf, size_t* len, size_t* id = nullptr, size_t* index = nullptr, int32_t timeout_millis = -1);
  ...
  */
  ...
  @return pointer to array[latest_index - 1].
  */
  void* GetLatestPtrNotify(size_t* id = nullptr, size_t* index = nullptr, size_t* size = nullptr,
                           int32_t timeout_millis = -1);
