...
private:
    void DrawDottedLine(cv::Mat &img, const cv::Point &pt1, const cv::Point &pt2, cv::Scalar color, int thickness,
    ...
    cv::Scalar GetLaneColor(LaneTypeEnum lane_type, bool dark = false);

    //cv::Scalar GetLaneColor(const std::vector<LaneDataType> &lanes, bool left, bool dark = false);
...
private:
    ...
    // color
    static const cv::Scalar LANE_COLOR_0;
    static const cv::Scalar LANE_COLOR_1;
    //static const cv::Scalar LANE_COLOR_2;

    static const cv::Scalar LANE_COLOR_RED;

    static const cv::Scalar LANE_COLOR_HIT;
    static const cv::Scalar LANE_COLOR_DEVIATE_0;
    static const cv::Scalar LANE_COLOR_DEVIATE_1;
    //static const cv::Scalar LANE_COLOR_DEVIATE_2;
