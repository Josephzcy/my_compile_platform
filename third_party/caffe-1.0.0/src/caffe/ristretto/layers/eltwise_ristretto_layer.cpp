...
EltwiseRistrettoLayer<Dtype>::EltwiseRistrettoLayer(const LayerParameter& param)
  ...
  case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT:
	...
	this->unsigned_in_ = this->layer_param_.quantization_param().unsigned_in();
	this->unsigned_out_ = this->layer_param_.quantization_param().unsigned_out();
    break;
...
template <typename Dtype>
void EltwiseRistrettoLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  CHECK(this->layer_param().eltwise_param().coeff_size() == 0
      || this->layer_param().eltwise_param().coeff_size() == bottom.size()) <<
      "EltwiseRistretto Layer takes one coefficient per bottom blob.";
  CHECK(!(this->layer_param().eltwise_param().operation()
      == EltwiseParameter_EltwiseOp_PROD
      && this->layer_param().eltwise_param().coeff_size())) <<
      "EltwiseRistretto layer only takes coefficients for summation.";
  ...
  if (this->layer_param().eltwise_param().coeff_size()) {
    for (int i = 0; i < bottom.size(); ++i) {
    	...
    	//if the coeficients are not fixed, then it will be transformed to fixed number
    	this->QuantizeLayerInputs_cpu(&(this->coeffs_[i]), 1);
    }
  }
  this->stable_prod_grad_ = this->layer_param_.eltwise_param().stable_prod_grad();
  //there are no quantized weights
}

template <typename Dtype>
void EltwiseRistrettoLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  if (this->layer_param_.eltwise_param().operation() ==
	  ...
	  this->max_idx_.Reshape(bottom[0]->shape());
  }
}
