...
void EltwiseRistrettoLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  case EltwiseParameter_EltwiseOp_SUM:
    ...
    for (int i = 0; i < bottom.size(); ++i) {
      caffe_gpu_axpy(count, this->coeffs_[i], bottom[i]->gpu_data(), top_data);
    }
    break;
  case EltwiseParameter_EltwiseOp_MAX:
    ...
    break;
  default:
    LOG(FATAL) << "Unknown elementwise ristretto operation.";
  }
  // Trim layer output
  //if (this->phase_ == TEST) {
    ...
    if (propagate_down[i]) {
      ...
      case EltwiseParameter_EltwiseOp_SUM:
        if (this->coeffs_[i] == Dtype(1.)) {
          ...
          caffe_gpu_scale(count, this->coeffs_[i], top_diff, bottom_diff);
        }
        break;
      case EltwiseParameter_EltwiseOp_MAX:
