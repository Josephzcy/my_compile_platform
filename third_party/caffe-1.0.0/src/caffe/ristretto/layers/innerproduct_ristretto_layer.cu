...
void InnerProductRistrettoLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  //std::cout<<this->M_<<std::endl<<this->bias_term_<<std::endl;
  if (this->M_ == 1) {
    caffe_gpu_gemv<Dtype>(CblasNoTrans, this->N_, this->K_, (Dtype)1.,
                         weight, bottom_data, (Dtype)0., top_data);
    //std::cout<<this->N_<<std::endl;
...
template <typename Dtype>
void InnerProductRistrettoLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(InnerProductRistrettoLayer);
