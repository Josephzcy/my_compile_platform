...
void BaseRistrettoLayer<Dtype>::QuantizeWeights_cpu(
  ...
  case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT: //weights always use signed quantization method
    Trim2FixedPoint_cpu(weight, cnt_weight, bw_params_, rounding, fl_params_, false);
    if (bias_term) {
...
//cut weights to zeros according to a fixed ratio
template <typename Dtype>
void BaseRistrettoLayer<Dtype>::CutWeights_cpu(vector<shared_ptr<Blob<Dtype> > > weights, float sparse_ratio)
{
     Dtype* weight = weights[0]->mutable_cpu_data();
     const int cnt_weight = weights[0]->count();
	 Dtype *weight_sorted = new Dtype[cnt_weight];
	 for (int i = 0; i < cnt_weight; i++)
		 weight_sorted[i] = fabs(weight[i]);
	 std::sort(weight_sorted, weight_sorted+cnt_weight);
	 int sparse_num = floor(sparse_ratio*cnt_weight);
	 float thresh = weight_sorted[sparse_num];
	 delete  [] weight_sorted;

	 for (int i = 0; i < cnt_weight; i++)
		 if (fabs(weight[i]) < thresh)
			 weight[i] = 0;
}
...
void BaseRistrettoLayer<Dtype>::QuantizeLayerInputs_cpu(Dtype* data,
  ...
  switch (precision_) {
    ...
    case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT:
      Trim2FixedPoint_cpu(data, count, bw_layer_in_, rounding_, fl_layer_in_, unsigned_in_);
      break;
    case QuantizationParameter_Precision_MINIFLOAT:
...
void BaseRistrettoLayer<Dtype>::QuantizeLayerOutputs_cpu(
  ...
  switch (precision_) {
    case QuantizationParameter_Precision_INTEGER_POWER_OF_2_WEIGHTS:
      break;
    case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT:
      Trim2FixedPoint_cpu(data, count, bw_layer_out_, rounding_, fl_layer_out_, unsigned_out_);
      break;
    case QuantizationParameter_Precision_MINIFLOAT:
      Trim2MiniFloat_cpu(data, count, fp_mant_, fp_exp_, rounding_);
      break;
...
inline double modified_round(double data)
{
	if ((data - floor(data)) == 0.5 && data<0)
	{
		data = round(data)+1;
	}
	else
  ...
  for (int index = 0; index < cnt; ++index) {
    ...
    case QuantizationParameter_Rounding_NEAREST:
        //data[index] = floor(data[index]); 
          data[index] = modified_round(data[index]);
      break;
...
//template int BaseRistrettoLayer<double>::qsort_compare(const void * a, const void * b);

template void BaseRistrettoLayer<double>::CutWeights_cpu(vector<shared_ptr<Blob<double> > > weights, float sparse_ratio);
template void BaseRistrettoLayer<float>::CutWeights_cpu(vector<shared_ptr<Blob<float> > > weights, float sparse_ratio);
