...
class FlowSend {
   public:
    ...
    int send(const char* topic, char* data, int size) {
        FLowSender->send(topic, data, size);
        return 0;
    }
    int send(const char* topic, char* data, int size, char* head, int head_size) {
    ...
    FlowRecv(const char* addr, const char* port, const char* topic,
        ...
        : flow::Client(
          ...
          FLowContext(flow::Config(
              {{"servers", "none"}, {"sender.serialize", "none"}})),
