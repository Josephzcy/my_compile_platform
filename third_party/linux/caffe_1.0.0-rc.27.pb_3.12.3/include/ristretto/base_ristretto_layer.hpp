...
class BaseRistrettoLayer{
 ...
 protected:
  ...
  void CutWeights_cpu(vector<shared_ptr<Blob<Dtype> > > weights, float sparse_ratio);
  ...
  void Trim2FixedPoint_cpu(Dtype* data, const int cnt, const int bit_width,
      const int rounding, int fl, bool unsigned_or_not);
  ...
  // activations.
  //only worked when layer is convolution, and tell the sparse ratio where to cut the weights to zero
  float sparse_ratio_;
  //parameters are always signed
  bool unsigned_in_, unsigned_out_;
